...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://tube.tor-ai.com", "http://localhost:8000"],
...
async def global_exception_handler(request: Request, exc: Exception):
    return JSONResponse(status_code=500, content={"detail": f"서버 오류: {str(exc)}"}, headers={"Access-Control-Allow-Origin": "https://tube.tor-ai.com"})

# --- DB 초기화 ---
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("CREATE TABLE IF NOT EXISTS video_cache (id INTEGER PRIMARY KEY AUTOINCREMENT, video_id TEXT UNIQUE, title TEXT, request_ip TEXT, transcript TEXT, summary TEXT, created_at TIMESTAMP, view_count INTEGER DEFAULT 1)")
        ...
        async with db.execute("SELECT id FROM users WHERE email = ?", (admin_id,)) as cursor:
            if not await cursor.fetchone():
                ...
                await db.execute("INSERT INTO users (email, hashed_password, name, is_admin, created_at) VALUES (?, ?, ?, ?, ?)", (admin_id, get_password_hash(admin_pw), "최고관리자", 1, datetime.datetime.now()))
        await db.commit()
    ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT id FROM ip_blocks WHERE ip = ?", (client_ip,)) as cursor:
                if await cursor.fetchone(): return JSONResponse(status_code=403, content={"detail": "차단된 IP"})
...
async def register(user: UserRegister):
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT id FROM users WHERE email = ?", (user.email,)) as cursor:
            if await cursor.fetchone(): raise HTTPException(status_code=400, detail="이미 존재")
...
async def login(user: UserLogin):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("SELECT * FROM users WHERE email = ?", (user.email,)) as cursor:
            ...
            if not db_user or not verify_password(user.password, db_user["hashed_password"]): raise HTTPException(status_code=400, detail="정보 불일치")
            return {"access_token": create_access_token(data={"sub": db_user["email"]}), "user": {"name": db_user["name"], "email": db_user["email"], "picture": db_user["profile_pic"], "is_admin": db_user["is_admin"]}}
...
async def auth_google(request: Request):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"https://www.googleapis.com/oauth2/v3/tokeninfo?id_token={google_token}")
        if resp.status_code != 200: raise HTTPException(status_code=400, detail="토큰 오류")
        user_info = resp.json()
    
    # 보안 검증: 발급된 토큰의 대상(aud)이 우리 Client ID와 일치하는지 확인
    if user_info.get("aud") != GOOGLE_CLIENT_ID:
        raise HTTPException(status_code=400, detail="유효하지 않은 클라이언트 요청입니다.")
...
async def download_subtitle(url, lang, is_auto):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            f = os.listdir(tmpdir)
            if f:
                with open(os.path.join(tmpdir, f[0]), 'r', encoding='utf-8') as file: return file.read()
        except: pass
    return None
